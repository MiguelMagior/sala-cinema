...
#define ARQUIVO_TEXTURA_PAREDE "TexturaParede.rgb"
...
*/
    ...
    glBindTexture(GL_TEXTURE_2D, id_textura_parede);
...

{
    ...
    img = ImageLoad(TEXTURA_DO_PLANO);

    if (img)

    {

        gluBuild2DMipmaps(GL_TEXTURE_2D, 3, img->sizeX, img->sizeY,
        ...
        // LIBERAR MEMÓRIA RAM (A imagem já foi para a Placa de Vídeo)
    ...
    // --- 2. TEXTURA DAS PAREDES ---

    glGenTextures(1, &id_textura_parede);

    glBindTexture(GL_TEXTURE_2D, id_textura_parede);



    img = ImageLoad(ARQUIVO_TEXTURA_PAREDE);
